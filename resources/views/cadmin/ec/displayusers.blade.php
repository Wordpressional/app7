...
@section('content')
    ...
    <main role="main" class="col-md-12 ml-sm-auto col-lg-12 pt-3 px-4">
      ...
      <a class="btn btn-sm btn-primary moveright" href="{{route('cadmin.polling.createeleusers')}}" style="margin-left:30px;">Create Users</a>

      <a class="btn btn-sm btn-primary moveright" href="{{route('cadmin.polling.listusers')}}">Map Users</a>
      <h2>{{$title}}</h2>
              ...
              @foreach($users as $key => $user)
               @if($user->roles[0]->name == "elec_ceo"  || $user->roles[0]->name == "elec_returningofficer"  || $user->roles[0]->name == "elec_presidingofficer"  || $user->roles[0]->name == "elec_sectorofficer" || $user->roles[0]->name == "elec_asistantreturningofficer" || $user->roles[0]->name == "elec_bootlevelofficer")
                  <tr>
                    ...
                    <td>
                      <div class="btn-group">
                        <a class="btn btn-primary" href="{{ route('cadmin.eleusers.editelec', ['id' => $user->id]) }}" class="btn btn-info btn-xs"><i class="fa fa-pencil" title="Role"></i> </a>
