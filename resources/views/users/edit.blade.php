    ...
    <div class="card-body">
      ...
      {!! Form::model($user, ['method' => 'PATCH', 'route' => ['users.update', $user]]) !!}
        ...
        <div class="form-group row">
          {!! Form::label('displayname', __('users.attributes.displayname'), ['class' => 'col-sm-2 col-form-label']) !!}

          <div class="col-sm-5">
            {!! Form::text('displayname', null, ['class' => 'form-control' . ($errors->has('displayname') ? ' is-invalid' : ''), 'placeholder' => __('users.placeholder.displayname'), 'required']) !!}

            @if ($errors->has('displayname'))
                <span class="invalid-feedback">{{ $errors->first('displayname') }}</span>
            @endif
          </div>
        </div>
