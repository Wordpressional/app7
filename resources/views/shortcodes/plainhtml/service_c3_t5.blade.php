<!-- Start service Area -->
			<section class="mpoltheme service-area section-gap" id="mpolservice">
				<div class="container boxmpoltheme4 textmpoltheme4">
					<span class="editmpoltheme4">edit</span>
                          
                           <span class="savempoltheme4">save</span>
					<div class="row justify-content-center">
						<div class="col-md-12 pb-50 header-text text-center">
							<h1 class="mb-10">What we Offer to our Supporters</h1>
							<p>
								Who are in extremely love with eco friendly system..
							</p>
						</div>
					</div>						
					<div class="row">
						<div class="col-lg-4">
							<div class="single-service">
								<div class="thumb boxmpoltheme5">
                                     
                                       <span class="editmpoltheme5">edit</span>
  
                           <span class="savempoltheme5">save</span>
                           <input type='file' id="imageUploadmpoltheme5" accept=".png, .jpg, .jpeg" />
                           <label class="imageUploadmpoltheme5" for="imageUploadmpoltheme5">&nbsp;</label>
        

									<img id="imagePreviewmpoltheme5" src="{{ url('themes/politt5/o1.jpg')}}" alt="">									
								</div>
								<h4>Conduct Campaign</h4>
								<p>
									inappropriate behavior is often laughed off as “boys will be boys,” women face higher conduct women face higher conduct.
								</p>
							</div>
						</div>
						<div class="col-lg-4">
							<div class="single-service">
								<div class="thumb boxmpoltheme6">
                                     
                                       <span class="editmpoltheme6">edit</span>
  
                           <span class="savempoltheme6">save</span>
                           <input type='file' id="imageUploadmpoltheme6" accept=".png, .jpg, .jpeg" />
                           <label class="imageUploadmpoltheme6" for="imageUploadmpoltheme6">&nbsp;</label>
        

									<img id="imagePreviewmpoltheme6" src="{{ url('themes/politt5/o2.jpg')}}" alt="">									
								</div>
								<h4>Generate Member</h4>
								<p>
									inappropriate behavior is often laughed off as “boys will be boys,” women face higher conduct women face higher conduct.
								</p>
							</div>
						</div>
						<div class="col-lg-4">
							<div class="single-service">
								<div class="thumb boxmpoltheme7">
                                     
                                       <span class="editmpoltheme7">edit</span>
  
                           <span class="savempoltheme7">save</span>
                           <input type='file' id="imageUploadmpoltheme7" accept=".png, .jpg, .jpeg" />
                           <label class="imageUploadmpoltheme7" for="imageUploadmpoltheme7">&nbsp;</label>
        

									<img id="imagePreviewmpoltheme7" src="{{ url('themes/politt5/o3.jpg')}}" alt="">									
								</div>
								<h4>Energy Saving Policy</h4>
								<p>
									inappropriate behavior is often laughed off as “boys will be boys,” women face higher conduct women face higher conduct.
								</p>
							</div>
						</div>												
					</div>
				</div>	
			</section>
			<!-- End service Area -->
